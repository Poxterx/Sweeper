class Room {
    ...
    create() {
        ...
        // La segunda capa en el archivo contendrá el suelo
        if (floor) {
            this.layers.push(this.map.createDynamicLayer(floor.name, this.tileset, 0, 0));
        }
        // La tercera capa en el archivo es la que debe contener las paredes. Las colisiones y el
        ...
        if (wall) {
            ...
            //this.layers[this.layers.length - 1].visible = false;
            ...
            for (let row = 0; row < this.map.height; row++) {
                ...
                for (let column = 0; column < this.map.width; column++) {
                    var currentTile = this.layers[this.layers.length - 1].getTileAt(column, row);
                    ...
                    currentTile = this.layers[this.layers.length - 1].getTileAt(column, row + 1);
                    if (currentTile && currentTile.properties.Vertical) {
        ...
        // La cuarta capa en el archivo es la que contiene las luces.
        // Nos aseguramos de asignarle una profundidad superior a la de las paredes,
        // en donde están colocadas.
        if (lights) {
        ...
        // La primera capa en el archivo es la que contiene el agua. Las colisiones y el
        // renderizado que se procesan más adelante parten del supuesto de que esta capa existe. Si
        // no es así, esta función ya no tiene nada más que hacer.
        if (water) {

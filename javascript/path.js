class Path {
    ...
    draw(graphics) {
        ...
        graphics.moveTo(this.entity.sprite.body.center.x - graphics.x, this.entity.sprite.body.center.y - graphics.y);
    ...
    static getNeighbourTiles(room, tile) {
        var neighbours = [
            room.getColliderTileAt({ x: tile.x, y: tile.y - 1 }),
            room.getColliderTileAt({ x: tile.x - 1, y: tile.y }),
            room.getColliderTileAt({ x: tile.x + 1, y: tile.y }),
            room.getColliderTileAt({ x: tile.x, y: tile.y + 1 })
        ];
        // Quizás algunos vecinos queden fuera de la sala. Antes de devolver el array
        // de vecinos es mejor comprobar expresamente cuáles están dentro.
        var ret = [];
        for (let tile of neighbours) {
            if (tile.x >= 0 && tile.x < room.size.x / TILE_SIZE
                && tile.y >= 0 && tile.y < room.size.y / TILE_SIZE) {
                ret.push(tile);
            }
        }
        return ret;
